...
def apply_auto_fixes(force_concise: bool) -> tuple[bool, bool]:
    ...
    if not force_concise:
        print(safe_emoji_text("🔧 Applying code formatting...", "Applying code formatting..."))  # noqa: T201
    ...
    if "reformatted" in format_output.lower():
        changes_made = True
        if not force_concise:
            print(safe_emoji_text("✅ Code formatting applied", "Code formatting applied"))  # noqa: T201

    # Step 2: Apply ruff style fixes (this will auto-fix style issues)
    if not force_concise:
        print(safe_emoji_text("🔧 Applying style fixes...", "Applying style fixes..."))  # noqa: T201
    ...
    if "fixed" in check_output.lower() or "Fixed" in check_output:
        changes_made = True
        if not force_concise:
            print(safe_emoji_text("✅ Style fixes applied", "Style fixes applied"))  # noqa: T201
...
def validate_quality(force_concise: bool) -> tuple[bool, bool, bool]:
    ...
    if not force_concise:
        print(safe_emoji_text("🔍 Validating code quality...", "Validating code quality..."))  # noqa: T201
...
def commit_auto_fixes(changes_made: bool, force_concise: bool) -> bool:
    ...
    if commit_changes(commit_msg):
        if not force_concise:
            print(safe_emoji_text("✅ Auto-fixes committed", "Auto-fixes committed"))  # noqa: T201
        return True

    if not force_concise:
        print(safe_emoji_text("❌ Failed to commit auto-fixes", "Failed to commit auto-fixes"))  # noqa: T201
    return False
...
def show_manual_fix_warnings(ruff_clean: bool, mypy_clean: bool, tests_clean: bool, force_concise: bool) -> None:
    ...
    if force_concise:
        print(safe_emoji_text("⚠️ MANUAL FIXES NEEDED", "MANUAL FIXES NEEDED"))  # noqa: T201
        print(f"Issues requiring manual fix: {', '.join(issues)}")  # noqa: T201
    else:
        print(safe_emoji_text("⚠️ Some issues require manual fixing:", "Some issues require manual fixing:"))  # noqa: T201
...
def main() -> int:
    ...
    if not force_concise:
        print(safe_emoji_text("🚀 Smart Auto-Fix and Validate", "Smart Auto-Fix and Validate"))  # noqa: T201
    ...
    if not fix_success:
        if force_concise:
            print(safe_emoji_text("❌ AUTO-FIX FAILED", "AUTO-FIX FAILED"))  # noqa: T201
        else:
            print(safe_emoji_text("❌ Auto-fix process failed", "Auto-fix process failed"))  # noqa: T201
        return 1
