...
class TestLongitudinalSection(unittest.TestCase):
    ...
    def _create_mock_segment_data(self, l=10.0, bz1=2.0, bz2=3.0, bz3=2.5, dz=0.5, dz_2=0.6) -> Munch:
        return Munch({
            "l": l,
            "bz1": bz1,
            "bz2": bz2,
    ...
    def _create_default_params(self, num_segments=1, section_loc_y=0.0):
        segments = [self._create_mock_segment_data(l=10 + i, dz=0.5 + i*0.1, dz_2=0.6 + i*0.1) for i in range(num_segments)]
        return Munch({
            "bridge_segments_array": segments,
            # Add other params if create_3d_model (mocked) or other parts of the function expect them
            # For now, keeping it minimal as create_3d_model will be mocked.
        })
    ...
    def test_create_longitudinal_section_basic_flow(self, mock_create_cross_section, mock_trimesh_module, mock_create_3d_model):
        ...
        mock_combined_2d_mesh.vertices = np.array([
            [0, 0, 0], [10, 0, 0], # Entity 1 (bottom line segment 1)
            [0, 0, 0.5], [10, 0, 0.5], # Entity 2 (top line segment 1)
            [10, 0, 0], [21, 0, 0], # Entity 3 (bottom line segment 2, l=11)
            [10, 0, 0.6], [21, 0, 0.6]  # Entity 4 (top line segment 2, dz=0.6)
        ])
        ...
        mock_entity1 = MagicMock(); mock_entity1.points = [0, 1]
        mock_entity2 = MagicMock(); mock_entity2.points = [2, 3]
        mock_entity3 = MagicMock(); mock_entity3.points = [4, 5]
        mock_entity4 = MagicMock(); mock_entity4.points = [6, 7]
        ...
        mock_create_cross_section.assert_called_once_with(
        ...
        self.assertEqual(len(fig.data), len(mock_combined_2d_mesh.entities)) # One trace per entity
        # Example check for the first trace (entity1)
        trace0 = fig.data[0]
        self.assertEqual(list(trace0.x), [mock_combined_2d_mesh.vertices[0][0], mock_combined_2d_mesh.vertices[1][0]]) # x-coords of points 0,1
        self.assertEqual(list(trace0.y), [mock_combined_2d_mesh.vertices[0][2], mock_combined_2d_mesh.vertices[1][2]]) # z-coords of points 0,1 (y in plot)
        ...
        # Check layout
        self.assertEqual(fig.layout.title.text, "Langsdoorsnede (Longitudinal Section)")
        self.assertEqual(fig.layout.xaxis.title.text, "X-as - Lengte [m]")
        self.assertEqual(fig.layout.yaxis.title.text, "Z-as - Hoogte [m]")
    ...
    @patch("src.geometry.longitudinal_section.create_cross_section")
    def test_create_longitudinal_section_annotations_detailed(self, mock_create_cross_section, mock_trimesh_module, mock_create_3d_model):
        ...
        params = Munch({
            "bridge_segments_array": [
                self._create_mock_segment_data(l=10, bz1=2, bz2=4, bz3=2, dz=0.5, dz_2=0.8),
                self._create_mock_segment_data(l=12, bz1=2, bz2=4, bz3=2, dz=0.6, dz_2=0.9),
            ]
        })
        ...
        mock_combined_2d_mesh.vertices = np.array([
            [0, 0, -0.5], [10, 0, -0.5], # Seg1 bottom (indices 0, 1)
            [0, 0, 0.0], [10, 0, 0.0],   # Seg1 top (indices 2, 3) (dz=0.5, so top is at 0 if bottom is -0.5)
            [10, 0, -0.6], [22, 0, -0.6], # Seg2 bottom (indices 4, 5) (l_cumulative=10, l=12 -> 22)
            [10, 0, 0.0], [22, 0, 0.0],   # Seg2 top (indices 6, 7) (dz=0.6, so top is at 0 if bottom is -0.6)
        ])
        # Define entities to cover all mock vertices to ensure SUT processes them for all_z
        mock_entity_s1_bottom = MagicMock(); mock_entity_s1_bottom.points = [0,1]
        mock_entity_s1_top = MagicMock(); mock_entity_s1_top.points = [2,3]
        mock_entity_s2_bottom = MagicMock(); mock_entity_s2_bottom.points = [4,5]
        mock_entity_s2_top = MagicMock(); mock_entity_s2_top.points = [6,7]
        mock_combined_2d_mesh.entities = [
            mock_entity_s1_bottom, mock_entity_s1_top,
            mock_entity_s2_bottom, mock_entity_s2_top
        ]
        mock_trimesh_module.util.concatenate.side_effect = [mock_combined_3d_mesh, mock_combined_2d_mesh]
        ...
        for entity in mock_combined_2d_mesh.entities:
            for ent_idx in entity.points:
                sut_processed_z_coords.append(mock_combined_2d_mesh.vertices[ent_idx][2])
        ...
        for ann in annotations:
            if ann.text in d_labels_texts:
                self.assertEqual(ann.x, d_labels_texts[ann.text])
                self.assertEqual(ann.y, max_z_plot + 0.5)
                self.assertEqual(ann.font, go.layout.Annotation(font={"size": 15, "color": "black"}).font)
                found_d_labels += 1
        self.assertEqual(found_d_labels, len(segments), "Incorrect number of D-labels")
        ...
        self.assertIsNotNone(ann_z2_1, f"Zone label {expected_zone_label_text} not found")
        if ann_z2_1:
            self.assertEqual(ann_z2_1.x, l_cumulative[0] + l_values[1]/2) # x-center of 2nd segment
            self.assertEqual(ann_z2_1.y, h_center_y_calc[1])
        ...
        self.assertIsNotNone(ann_len_s2, f"Length annotation {expected_len_text_s2} not found")
        if ann_len_s2:
            self.assertEqual(ann_len_s2.x, l_cumulative[0] + l_values[1]/2)
            self.assertAlmostEqual(ann_len_s2.y, min_z_plot - 1.0) # Uses corrected min_z_plot
            self.assertEqual(ann_len_s2.font.color, "red")
        ...
        self.assertIsNotNone(ann_h_s1, f"Height annotation {expected_h_text_s1} not found")
        if ann_h_s1:
            self.assertEqual(ann_h_s1.x, l_cumulative[0] - 0.5)
            self.assertEqual(ann_h_s1.y, h_center_y_calc[0])
            self.assertEqual(ann_h_s1.font.color, "blue")
            self.assertEqual(ann_h_s1.textangle, -90)

        expected_h_text_s2 = f"<b>h = {segments[1].dz}m</b>"
        ann_h_s2 = next((a for a in annotations if a.text == expected_h_text_s2), None)
        self.assertIsNotNone(ann_h_s2, f"Height annotation {expected_h_text_s2} not found")
        if ann_h_s2:
            self.assertEqual(ann_h_s2.x, l_cumulative[1] - 0.5)
            self.assertEqual(ann_h_s2.y, h_center_y_calc[1])
