...
from src.common.gis_utils import prepare_bridge_data_for_viktor, get_map_center_and_zoom, load_bridge_shapefile #, load_bridge_shapefile
...
class TestGisUtilsLoadBridgeShapefile(unittest.TestCase):
    ...
    def test_load_shapefile_with_multiple_filters(self, mock_gpd_read_file):
        ...
        def getitem_initial_gdf(key):
            if key == "COL1": return col1_series
            if key is bool_series1: return mock_gdf_after_filter1
        ...
        def getitem_gdf_after_filter1(key):
            if key == "COL2": return col2_series
            if key is bool_series2: return mock_gdf_after_filter2
        ...
        self.assertTrue(any(call_args[0] == "COL1" for call_args, _ in mock_gdf_initial.__getitem__.call_args_list) or \
                        any(call_args[0] is bool_series1 for call_args, _ in mock_gdf_initial.__getitem__.call_args_list))
    ...
    def test_load_shapefile_read_file_raises_exception(self, mock_gpd_read_file):
        ...
        mock_gpd_read_file.side_effect = IOError("File not found")
